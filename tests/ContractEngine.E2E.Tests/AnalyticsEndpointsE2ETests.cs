...
/// <summary>
/// Boots the compiled Api DLL over Kestrel on dedicated port 5058 and exercises the Batch 017
/// analytics + health endpoints end-to-end: register → counterparty → contract → obligation →
/// GET /api/analytics/dashboard → verify pending_obligations incremented, plus /health/db and
/// /health/ready round-trips through the real middleware pipeline.
/// </summary>
public class AnalyticsEndpointsE2ETests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task Analytics_DashboardAndHealth_RoundTripThroughRealKestrel()
    {
        // Register tenant.
        var registerResp = await _client!.PostAsJsonAsync("/api/tenants/register", new
        {
            name = $"E2E Analytics {Guid.NewGuid()}",
        });
        ...
        {
            name = $"E2E-Analytics-CP {Guid.NewGuid()}",
        });
        ...
        {
            title = $"E2E Analytics Contract {Guid.NewGuid()}",
        ...
        var oblResp = await authed.PostAsJsonAsync("/api/obligations", new
        {
            ...
            title = "E2E Analytics Obligation",
            deadline_date = "2026-12-01",
        });
        oblResp.StatusCode.Should().Be(HttpStatusCode.Created);

        // GET /api/analytics/dashboard — we just created one pending obligation.
        var dashResp = await authed.GetAsync("/api/analytics/dashboard");
        dashResp.StatusCode.Should().Be(HttpStatusCode.OK);
        using var dashDoc = JsonDocument.Parse(await dashResp.Content.ReadAsStringAsync());
        dashDoc.RootElement.GetProperty("pending_obligations").GetInt32().Should().BeGreaterOrEqualTo(1);

        // /health/db without auth.
        var healthDbResp = await _client.GetAsync("/health/db");
        healthDbResp.StatusCode.Should().Be(HttpStatusCode.OK);
        using var healthDbDoc = JsonDocument.Parse(await healthDbResp.Content.ReadAsStringAsync());
        healthDbDoc.RootElement.GetProperty("status").GetString().Should().Be("healthy");

        // /health/ready without auth.
        var readyResp = await _client.GetAsync("/health/ready");
        readyResp.StatusCode.Should().Be(HttpStatusCode.OK);
        using var readyDoc = JsonDocument.Parse(await readyResp.Content.ReadAsStringAsync());
        readyDoc.RootElement.GetProperty("status").GetString().Should().Be("ready");
        readyDoc.RootElement.GetProperty("database").GetString().Should().Be("healthy");
    }
